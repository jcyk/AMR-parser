...
class AMR(object):
    ...
    def rename_node(self, prefix):
        ...
        if self.root is not None:
            self.root = node_map_dict[self.root]
  
    def get_triples(self, levels=-1, weighted=False, prefix=None):
        """
        ...
        """
        if not (levels== -1 and weighted == False):
            return AMRGraph(self).trim(levels, weighted, prefix)

        if prefix is not None:
            self.rename_node(prefix)

        instance_triple = []
        relation_triple = []
        attribute_triple = []
        for i in range(len(self.nodes)):
            instance_triple.append(("instance", self.nodes[i], self.node_values[i], 1))
            # k is the other node this node has relation with
            # v is relation name
            for k, v in self.relations[i].items():
                relation_triple.append((v, self.nodes[i], k, 1))
            # k2 is the attribute name
            # v2 is the attribute value
            for k2, v2 in self.attributes[i].items():
                attribute_triple.append((k2, self.nodes[i], v2, 1))
        return instance_triple, attribute_triple, relation_triple, len(instance_triple)+len(attribute_triple)+len(relation_triple)
