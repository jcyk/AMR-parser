...
def build_arg_parser():
    ...
    parser.add_argument('--weighted', action='store_true', help="whether use a weighted smatch or not")
    parser.add_argument('--levels', type=int, default=-1, help="how deep you want to evaluate, -1 indicates unlimited, i.e., full graph")
    parser.add_argument('--max_size', type=int, default=-1, help="only consider AMR grpahs with limited size <=, -1 indicates unlimited")
    parser.add_argument('--min_size', type=int, default=-1, help="only consider AMR grpahs with limited size >=, -1 indicates unlimited")
        ...
        for j in range(0, len(instance2)):
            ...
            if instance1[i][0].lower() == instance2[j][0].lower() \
                ...
                if node_pair in weight_dict:
                    assert False, "???????"
                    weight_dict[node_pair][-1] += min(instance2[j][-1], instance1[i][-1])
                else:
    ...
    for i in range(0, len(attribute1)):
        for j in range(0, len(attribute2)):
            ...
            if attribute1[i][0].lower() == attribute2[j][0].lower() \
                ...
                # use -1 as key in weight_dict for instance triples and attribute triples
                if node_pair in weight_dict:
                    weight_dict[node_pair][-1] += min(attribute2[j][-1], attribute1[i][-1])
                else:
                    weight_dict[node_pair] = {}
                    weight_dict[node_pair][-1] = min(attribute2[j][-1], attribute1[i][-1])
                ...
                if node_pair2 != node_pair1:
                    ...
                    if node_pair1 in weight_dict:
                        if node_pair2 in weight_dict[node_pair1]:
                            weight_dict[node_pair1][node_pair2] += min(relation2[j][-1], relation1[i][-1])
                        else:
                            weight_dict[node_pair1][node_pair2] = min(relation2[j][-1], relation1[i][-1])
                    else:
                    ...
                    if node_pair2 in weight_dict:
                        if node_pair1 in weight_dict[node_pair2]:
                            weight_dict[node_pair2][node_pair1] += min(relation2[j][-1], relation1[i][-1])
                        else:
                            weight_dict[node_pair2][node_pair1] = min(relation2[j][-1], relation1[i][-1])
                    else:
                    ...
                    if node_pair1 in weight_dict:
                        weight_dict[node_pair1][-1] += min(relation2[j][-1], relation1[i][-1])
                    else:
                        weight_dict[node_pair1] = {}
                        weight_dict[node_pair1][-1] = min(relation2[j][-1], relation1[i][-1])
...
def main(arguments):
    ...
    while True:
        ...
        prefix1 = 'a'
        prefix2 = 'b'
        (instance1, attributes1, relation1, test_triple_num) = amr1.get_triples(arguments.levels, arguments.weighted, prefix=prefix1)
        (instance2, attributes2, relation2, gold_triple_num) = amr2.get_triples(arguments.levels, arguments.weighted, prefix=prefix2)
        if arguments.max_size != -1 and len(instance2)+len(attributes2) -1 > arguments.max_size:
            continue
        if arguments.min_size != -1 and len(instance2)+len(attributes2) -1 < arguments.min_size:
            continue
        if verbose:
