...
from transformer import Transformer, Embedding, SinusoidalPositionalEmbedding, SelfAttentionMask
from data import ListsToTensor, ListsofStringToTensor, DUM
...
class Parser(nn.Module):
    def __init__(self, vocabs, word_char_dim, word_dim, pos_dim, ner_dim,
        ...
        self.word_encoder = WordEncoder(vocabs,
                        ...
                        embed_dim, cnn_filters, char2word_dim, dropout, pretrained_word_embed)

        self.concept_encoder = ConceptEncoder(vocabs,
                                          concept_char_dim, concept_dim, embed_dim,
                                          cnn_filters, char2concept_dim, dropout)
    ...
    def forward(self, data):
        ...
        return self.decoder(probe, word_repr, concept_repr, word_mask, concept_mask, attn_mask, \
         data['copy_seq'], target=data['concept_out'], target_rel=data['rel'])
